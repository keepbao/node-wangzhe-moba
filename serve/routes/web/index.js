module.exports = app => {
  ...
  router.get('/news/init', async (req, res) => {
    ...
    const newsTitles = ["第三届王者荣耀全国大赛——安徽省再次启航！", "第三届王者荣耀全国大赛城市赛道——第一期海选赛赛点信息", "斗鱼新势力战队选拔赛", "狄某有话说｜江湖规矩，对线不打“WiFi”牛", "老亚瑟的答疑时间：貂蝉-仲夏夜之梦及金色仲夏夜优化后海报展示", "3月13日体验服不停机更新公告", "3月12日体验服停机更新公告", "第三届王者荣耀全国大赛城市赛道——第一期海选赛赛点信息", "3月12日体验服停机更新公告", "3月10日净化游戏环境声明及处罚公告", "【心动纸飞机】活动公告", "三月白情恰花开，峡谷好礼携春来", "【峡谷女神的细节考验】活动公告", "女神节来三月春，峡谷好礼迎春来", "元宵佳节至，峡谷迎春来", "第三届王者荣耀全国大赛——安徽省再次启航！", "第三届王者荣耀全国大赛城市赛道——第一期海选赛赛点信息", "2021年KPL春季赛季前赛今日开赛！重磅转会选手，新战场首秀来袭！", "K甲春季赛线下售票规则", "2021年K甲春季赛季前赛3月4日12:00开启售票，全新主场，为更强喝彩！"]
  ...
  router.get('/news/list', async (req, res) => {
    // 出来的数据是按照id自行分配的，无法规定每页展示五条数据的规则
    // const parent = await Category.findOne({
    //   name: 'news'
    // }).populate({
    //   path: 'children',
    ...
    const cats = await Category.aggregate([
      //定义match操作符，条件查询类似于mysol的WHERE方法
      {
        $match: {
          parent: parent._id
        }
      },
      // 操作符$lookup类似于关系型数据库的join，连接
      {
        $lookup: {
          // 数据库创建时会默认将文档名设置为复数，如果不需要可以在定义的时候指定文档名
          from: 'articles',
          localField: '_id', //本地键
  ...
  router.get('/heroes/init', async (req, res) => {
    ...
    for (let cat of rawData) {
      ...
      // continue 表示跳过  直接执行下一轮循环，不要执行后续代码  js的关键字
      // if (cat.name === '热门') {
      //   continue
      // }
      // 找到当前分类在数据库中对应的数据
